...
if(empty($wd_id) || empty($wd_first_day) || empty($wd_last_day) || empty($wd_dept) || empty($wd_job) || 
  ...
  if($first_day >= $last_day) {
    ...
    if($check_count == 1) {
      ...
      $sql = "UPDATE `work_detail` SET `wd_week`='$wd_week', `wd_first_day`='$wd_first_day', `wd_last_day`='$wd_last_day'
              `wd_dept`=$wd_dept', `wd_job`=$wd_dept', `wd_issue`='$wd_issue', `wd_fix`='$wd_fix', `wd_exp`='$wd_exp', 
              `wd_solve`='$wd_solve', `wd_comment`='$wd_comment'
              WHERE `wd_id`='$wd_id' AND `std_id`='$std_id' ";
